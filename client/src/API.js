  ...
  if (response.ok) {
    ...
    if (latest && JsonServices.length > 0) {
      ...
      if (JsonServices.length === 1)
        return JsonServices[0];

      let servingTicketFiltered = JsonServices[0];

      for (let i = 0; i < JsonServices.length; i++) {
        if (JsonServices[i].timestamp > servingTicketFiltered.timestamp) {
          servingTicketFiltered = JsonServices[i];
        }
      }

      return { ...(!!servingTicketFiltered ? servingTicketFiltered : servingTicketDefault) };
    } else {
      return JsonServices
    }
  }
