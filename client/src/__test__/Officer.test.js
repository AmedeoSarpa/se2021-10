...
it("Officer Page props passed correctly", () => {
    const wrapper = mount(<OfficerPage
        services={services}
        queues={queues}
        counterService={counterService}
        processRequest={processRequest}
    ...
    expect(wrapper.props().processRequest).toEqual(processRequest);
    expect(wrapper.props().counterId).toEqual(counterId);

});

it("renders OfficerPage header", () => {
    const wrapper = mount(<OfficerPage
        services={services}
        queues={queues}
        counterService={counterService}
        processRequest={processRequest}
        counterId={counterId} />);
    const name = <h1>Officer</h1>;
    expect(wrapper.contains(name)).toEqual(true);
});
/*
it('renders OfficerPage next client button', () => {
    const wrapper = mount(<OfficerPage
        services={services}
        queues={queues}
        counterService={counterService}
        processRequest={processRequest}
        counterId={counterId} />)
    const buttonElement  = wrapper.find('btn btn-primary btn-lg btn-block');
    expect(buttonElement.text()).toEqual('NEXT CLIENT');
  })
   */
